...
- **分頁隱藏時輪詢暫停**,回前景自動補刷(2026-07-10:遺忘的背景分頁曾把整個
  free fleet 當 keepalive 燒池;詳見下方探測衰減)。
...
- **探測衰減**:連續輪詢 30 分鐘無互動(無 ↺ Now、無重開分頁)→ 探測間隔自動升到
  **20 分鐘**(> Render 15min spindown,讓 can-sleep 服務真的能睡)。回來看一眼
  (輪詢中斷後恢復)或按 ↺ Now 即回全速。status 的 `renderProbeMode` 顯示
  attended/idle 目前模式。
...
| 資料 | 節奏 | 手動強制 |
|------|------|----------|
| 服務健康 / 本頁整體 | 每 10s(隱藏分頁暫停;或按 `R`) | ↺ Refresh |
| Render 探測快取 | ≤ 1 次/60s(30min 無互動衰減至 20min) | ↺ Now |
