...
export default function Sidebar({ projects, selectedId, onSelect, onDashboard, onAdmin, onMarketing, onDecisions, onPortfolio, onTemplates, onNewProject, view, collapsed, onToggleCollapse }) {
    ...
    <div className={`sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      <div className="sidebar-section">
        ...
        <div
          ...
          onClick={onPortfolio}
          title={collapsed ? t.portfolioTitle : undefined}
        >
          <span style={{ fontSize: 14 }}>🗂</span>
          <span className="name">{t.portfolioTitle}</span>
        </div>
        <div
          className={`sidebar-item ${view === 'decisions' ? 'active' : ''}`}
          onClick={onDecisions}
          title={collapsed ? t.decisionLogTitle : undefined}
        >
          <span style={{ fontSize: 14 }}>🧭</span>
          <span className="name">{t.decisionLogTitle}</span>
        </div>
        <div
          className={`sidebar-item ${view === 'templates' ? 'active' : ''}`}
          onClick={onTemplates}
          title={collapsed ? t.templatesTitle : undefined}
        >
          <span style={{ fontSize: 14 }}>📄</span>
          <span className="name">{t.templatesTitle}</span>
        </div>
        <div
          className={`sidebar-item ${view === 'marketing' ? 'active' : ''}`}
          onClick={onMarketing}
