...
export default function ProjectDetail({
  ...
  const [taskForm, setTaskForm] = useState(null) // null | { status, task? }
  ...
  const handleDrop = (colKey) => {
    ...
    if (task && task.status !== colKey) {
      onUpdateTask(draggingId, { status: colKey })
    }
    setDraggingId(null)
    setDragOver(null)
  }
    ...
    <div className="project-detail">
      ...
      <div className="project-header">
        <div className="flex items-center gap-8">
          ...
          <button className="btn btn-ai btn-sm" onClick={() => setShowAI(true)}>✨ AI Assistant</button>
          <button className="btn btn-sm" onClick={onEditProject}>Edit</button>
        ...
        <div className="project-meta-row" style={{ marginTop: 8 }}>
          ...
          {project.dueDate && <span className="text-muted text-sm">Due {fmtDate(project.dueDate)}</span>}
          <span className="text-muted text-sm ml-auto">{done}/{total} tasks · {pct}%</span>
        </div>
      ...
      <div className="kanban">
        {COLUMNS.map(col => {
          const colTasks = tasks.filter(t => t.status === col.key).sort((a, b) => a.sortOrder - b.sortOrder)
          return (
...
function TaskCard({ task, isDragging, onDragStart, onEdit, onDelete, onStatusChange }) {
  const today = new Date().toISOString().split('T')[0]
  ...
  return (
    ...
    >
      ...
      <div className="task-meta">
        ...
        {task.dueDate && <span className={`task-due ${cls}`}>{fmtDate(task.dueDate)}</span>}
